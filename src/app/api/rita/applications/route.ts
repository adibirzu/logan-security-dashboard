...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('RITA Applications API: Analyzing applications for', { timeRange, timePeriodMinutes })

    // Get application communications analysis
    ...
    if (result.success) {
      ...
      return NextResponse.json({
        success: false,
        error: result.error || 'Application analysis failed',
        applications: []
      }, { status: 500 })
    }
  } catch (error) {
    console.error('RITA Applications API Error:', error)
    return NextResponse.json({
      success: false,
