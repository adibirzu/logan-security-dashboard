...
class ScreenshotServer {
      ...
      try {
        switch (name) {
          case "take_screenshot":
            return await this.takeScreenshot(args as unknown as ScreenshotConfig);
  ...
  private async addAnnotations(inputPath: string, outputPath: string, annotations: Annotation[]) {
    // Get image metadata
      ...
      switch (annotation.type) {
        case "arrow":
          if (annotation.toX && annotation.toY) {
            const angle = Math.atan2(annotation.toY - annotation.y, annotation.toX - annotation.x);
  ...
  private async annotateScreenshot(args: { imagePath: string; outputPath: string; annotations: Annotation[] }) {
